...
var options = {
	ops:{
		"input":[
			...
			,{"name":"Test","label":"test config","type":"button","className":"displayonly",value:"validate"}
	...
	setupForm:function(){
		for (var i = 0; i<options.ops.input.length; i++){
			...
			if (item.type === "button"){
					$('form fieldset')
						.append('<p><label for="'+item.name+'">'+ item.label +'</label><input class="'+ item.className+'" '+ opsStr +' type="'+ type +'" id="'+ item.name +'" value="'+ value +'" /></p>')
					...
					if (httpRequest.readyState === 4) {
						if (httpRequest.status === 200) {
							...
							if (data && data.jobs && data.jobs.length){
								...
								alert('Connected OK, but no jobs returned.\nYou might need to open another tab and login to your Jenkins server')
							}
						} else {
