...
function init() {
	...
	if (jobs && jobs.length) {
		...
		for (var i = 0; i < jobs.length ; i++) {
			...
			anime = (colour.match("_anime")) ? "anime" : "static";
			...
			try {
				$(newRow).append($('<div id="'+name.id+'" class="unit td '+ colour +' '+result+' '+anime+'"><div class="top">'+img+'</div><h2><a href="'+ job.url +'">'+name.display+'</a></h2><p><a href="'+ lastBuild.url +'">'+result+': '+displayDate+'</a></p><p class="culprit">'+getComment(culprit,job.color)+'</p></div>'));
